...
class YoutubeMp3Downloader extends EventEmitter {
    ...
    async performDownload(task, callback) {
        ...
        stream.on('response', function(httpResponse) {
            ...
            let audioBitrateIndex = 0
            let audioBitrate = info.formats[audioBitrateIndex].audioBitrate
            while (!audioBitrate) {
              audioBitrateIndex++
              if (info.formats.lenght === audioBitrateIndex) {
                audioBitrate = 96
              }
              audioBitrate = info.formats[audioBitrateIndex].audioBitrate
            }
